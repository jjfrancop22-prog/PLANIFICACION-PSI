...
import {qualityDeleteGate} from './security/quality-delete-gate.js';
import {qualityCorrectionGate} from './security/quality-correction-gate.js';
import {specialAnalysisCorrectionGate} from './security/special-analysis-correction-gate.js';
...
function currentRoleId(){return String(securityManager.sessions.current()?.role||'').trim().toUpperCase()}
function isBillingMonitoringReadOnly(){return currentRoleId()==='FACTURACION'}
function billingCanReadMonitoringView(viewId,owner){return isBillingMonitoringReadOnly()&&owner==='monitoring'&&['registry','analysis','waiting','stopped'].includes(viewId)}
function table(rows,kind){if(!rows.length)return '<div class="empty">No hay registros en esta bandeja.</div>';const billingReadOnly=isBillingMonitoringReadOnly();const showFrequency=kind==='registry'||kind==='analysis',showAnalysisSelect=kind==='analysis'&&analysisBulkMode&&!billingReadOnly;return `<div class="table-wrap"><table><thead><tr>${showAnalysisSelect?'<th style="width:42px"><input type="checkbox" id="analysisSelectAll" title="Seleccionar visibles"></th>':''}<th>Código</th><th>Año</th><th>Cliente / Sucursal</th><th>Matriz / Grupo</th><th>Fecha muestra</th><th>Recepción</th>${showFrequency?'<th>Frecuencia / mes</th>':''}<th>DQO/Tenso</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows.map(x=>{const req=x.workflow?.requirements||{},vals=x.workflow?.analysisValues||{},hasSpecial=!!(req.dqo||req.surfactants);let actions=billingReadOnly?`<button class="btn secondary small" data-history="${x.id}">Historial</button>`:`<button class="btn blue small" data-edit="${x.id}">Editar</button> ${kind==='registry'&&x.workflow?.decisionStatus==='CONTINUAR'?`<button class="btn warning small" data-quality-stop="${x.id}">Corregir a DETENIDA</button> `:''}<button class="btn secondary small" data-history="${x.id}">Historial</button> <button class="btn danger small" data-safe-delete-sample="${x.id}" data-safe-delete-stage="MONITORING">Eliminar</button>`;if(!billingReadOnly&&kind==='analysis')actions=`<button class="btn primary small" data-analysis="${x.id}">${hasSpecial?'Registrar análisis':'Registrar análisis · continuar'}</button> ${actions}`;if(!billingReadOnly&&kind==='waiting')actions=`<button class="btn warning small" data-waiting="${x.id}">Resolver</button> ${actions}`;return `<tr>${showAnalysisSelect?`<td><input type="checkbox" data-analysis-select="${x.id}" ${analysisSelected.has(x.id)?'checked':''}></td>`:''}<td><b>${esc(x.code)}</b><div class="muted mono">${esc(x.id.slice(0,8))}…</div></td><td>${esc(x.year)}</td><td><b>${esc(x.clientId)}</b><div class="muted">${esc(x.branch||'—')}</div></td><td><b>${esc(x.matrixId)}</b><div class="muted">${esc(x.groupId)}</div></td><td>${formatDate(x.samplingDate)}</td><td>${formatDate(x.receivedDate)}</td>${showFrequency?`<td><b>${esc(x.monthFrequency||'—')}</b></td>`:''}<td>${req.dqo?`DQO: ${esc(vals.dqo||'pendiente')}`:'DQO: —'}<br>${req.surfactants?`Tenso: ${esc(vals.surfactants||'pendiente')}`:'Tenso: —'}</td><td>${statusBadge(x)}</td><td>${actions}</td></tr>`}).join('')}</tbody></table></div>`}
...
async function loadCatalogs(){clients=await service.clients();matrices=await service.matrices();$('clientList').innerHTML=clients.map(c=>`<option value="${esc(c.name)}"></option>`).join('');$('matrix').innerHTML=matrices.map(m=>`<option value="${m.id}">${esc(m.label||m.name)}</option>`).join('');const residual=matrices.find(m=>String(m.name||m.label||'').trim().toUpperCase()==='RESIDUAL');if(residual&&!$('editId')?.value)$('matrix').value=residual.id;updateBranches();renderClientCatalog();renderMatrixCatalog();renderFilterOptions();matrixChanged()}
...
function daysUntil(date){if(!date)return null;return businessDaysBetween(new Date().toISOString().slice(0,10),date)}
...
async function openLabEntry(sampleId){try{const sample=allRows.find(x=>x.id===sampleId);if(!sample)throw new Error('Muestra no encontrada.');const entry=await labService.prepare(sampleId,{userId:'LOCAL_USER'});$('labSampleId').value=sampleId;$('labEntryId').value=entry.id;$('labEntrySubtitle').textContent=`${sample.code} · ${sample.year}`;$('labReadonlyClient').textContent=sample.clientId||'—';$('labReadonlyBranch').textContent=sample.branch||'—';$('labReadonlyMatrix').textContent=`${sample.matrixId||'—'}${sample.groupId?' · '+sample.groupId:''}`;await loadLabAnalystOptions();$('labDecisionNotice').innerHTML=sample.workflow?.decisionStatus==='DETENIDA'?'<b>Atención:</b> esta muestra está DETENIDA. Se mantiene visible en Laboratorio por trazabilidad y puede registrarse su ingreso oficial.':'<b>Flujo:</b> muestra habilitada para ingreso oficial al Laboratorio.';$('labDecisionNotice').classList.toggle('warn',sample.workflow?.decisionStatus==='DETENIDA');$('labReceptionDate').value=entry.receptionDate||'';$('labAnalyst').value=entry.analyst||'';$('labServiceType').value=entry.serviceType||'INTERNO';$('labSamplingMonth').value=entry.samplingMonth||'';$('labSlaDays').value=slaDaysForService(entry.serviceType||'INTERNO');$('labMaxReportDate').value=addLabDays(entry.receptionDate||'',Number($('labSlaDays').value));$('labEntryModal').classList.add('show')}catch(e){toast(e.message||String(e),true)}}
function addLabDays(date,days){return addBusinessDays(date,days)}
...
function renderTables(){const aq=$('searchAnalysis').value.trim().toLowerCase(),wq=$('searchWaiting').value.trim().toLowerCase(),sq=$('searchStopped').value.trim().toLowerCase();const analysisRows=queues.analysis.filter(x=>hay(x,aq));$('analysisTable').innerHTML=table(analysisRows,'analysis');$('waitingTable').innerHTML=table(queues.waiting.filter(x=>hay(x,wq)),'waiting');$('stoppedTable').innerHTML=table(queues.stopped.filter(x=>hay(x,sq)),'stopped');const r=filteredRegistry();$('registryTable').innerHTML=table(r,'registry');$('registryResultCount').textContent=`${r.length} visible${r.length===1?'':'s'}`;renderAnalysisBulkControls(analysisRows)}
function renderAnalysisBulkControls(visibleRows=[]){const panel=$('analysisBulkPanel'),toggle=$('analysisBulkToggle'),count=$('analysisSelectedCount');if(!panel||!toggle)return;panel.style.display=analysisBulkMode?'flex':'none';toggle.textContent=analysisBulkMode?'Cancelar selección múltiple':'☑ Selección múltiple';if(count)count.textContent=`${analysisSelected.size} seleccionada${analysisSelected.size===1?'':'s'}`;const all=$('analysisSelectAll');if(all&&visibleRows.length){const visibleIds=visibleRows.map(x=>x.id);all.checked=visibleIds.every(id=>analysisSelected.has(id));all.indeterminate=!all.checked&&visibleIds.some(id=>analysisSelected.has(id))}}
function toggleAnalysisBulk(){analysisBulkMode=!analysisBulkMode;if(!analysisBulkMode)analysisSelected.clear();renderTables()}
async function applyAnalysisReceptionDate(){try{const date=$('analysisBulkDate').value,ids=[...analysisSelected];if(!ids.length)throw new Error('Seleccione al menos una muestra.');if(!date)throw new Error('Seleccione la fecha de recepción.');await service.updateReceptionDates(ids,date,{userId:'LOCAL_USER'});analysisSelected.clear();analysisBulkMode=false;$('analysisBulkDate').value='';toast(`Fecha de recepción aplicada a ${ids.length} muestra(s). Permanecen en Registro de Análisis.`);await refresh();showView('analysis')}catch(e){toast(e.message||String(e),true)}}
async function directAnalysisContinue(id){try{await service.registerAnalysis(id,{dqo:'',surfactants:''},{userId:'LOCAL_USER'});toast('Muestra sin DQO/Tenso: ingreso de análisis confirmado y enviada a Registro de Muestras/Laboratorio.');await refresh();showView('analysis')}catch(e){toast(e.message||String(e),true)}}
...
function clearForm(){$('sampleForm').reset();$('editId').value='';$('year').value=new Date().getFullYear();$('samplingDate').value=new Date().toISOString().slice(0,10);$('saveBtn').textContent='Guardar muestra';$('requiresSpecialAnalysis').disabled=false;const residual=matrices.find(m=>String(m.name||m.label||'').trim().toUpperCase()==='RESIDUAL');if(residual)$('matrix').value=residual.id;showError();matrixChanged();$('code').focus()}
...
async function submit(e){e.preventDefault();showError();try{const id=$('editId').value;const data=formData();if(id){const current=allRows.find(x=>x.id===id);const beforeSpecial=!!(current?.workflow?.requirements?.dqo||current?.workflow?.requirements?.surfactants);const afterSpecial=!!(data.requiresDqo||data.requiresSurfactants);let userId='LOCAL_USER';if(beforeSpecial!==afterSpecial){if(!await specialAnalysisCorrectionGate.authorize({entityId:id,label:`la muestra ${current?.code||current?.codeFull||''}`,from:beforeSpecial,to:afterSpecial}))return;userId=specialAnalysisCorrectionGate.actor()}await service.updateRegistry(id,data,{userId,allowRequirementChange:beforeSpecial!==afterSpecial});toast(beforeSpecial!==afterSpecial?'Datos actualizados y cambio DQO/Tenso autorizado por Calidad.':'Datos de la muestra actualizados localmente.')}else{const saved=await service.register(data,{userId:'LOCAL_USER'});toast(saved.workflow.workflowStage==='ANALYSIS_REGISTRATION'?'Muestra guardada. Pasa a Registro de Análisis.':'Muestra guardada. Ingreso culminado.')}clearForm();await refresh();if(!id&&allRows[0]?.workflow?.workflowStage==='ANALYSIS_REGISTRATION')showView('analysis')}catch(err){showError(err.message||String(err));toast(err.message||String(err),true)}}
...
function openAnalysis(id){const s=allRows.find(x=>x.id===id);if(!s)return;const req=s.workflow?.requirements||{};if(!req.dqo&&!req.surfactants){directAnalysisContinue(id);return}$('analysisId').value=id;$('analysisTitle').textContent=`Análisis · ${s.code}`;$('analysisSubtitle').textContent=`${s.clientId} · ${s.matrixId}`;$('dqoValue').disabled=!req.dqo;$('surfactantValue').disabled=!req.surfactants;$('dqoValue').value=s.workflow.analysisValues?.dqo||'';$('surfactantValue').value=s.workflow.analysisValues?.surfactants||'';$('analysisModal').classList.add('show')}
...
function openEdit(id){const s=allRows.find(x=>x.id===id);if(!s)return;showView('register');$('editId').value=id;$('code').value=s.code;$('year').value=s.year;$('client').value=s.clientId;$('branch').value=s.branch||'';const matrix=matrices.find(m=>m.id===s.matrixCatalogId)||matrices.find(m=>m.name===s.matrixId);if(matrix)$('matrix').value=matrix.id;$('samplingDate').value=s.samplingDate||'';$('receivedDate').value=s.receivedDate||'';$('monthFrequency').value=s.monthFrequency||'';$('observations').value=s.observations||'';$('requiresSpecialAnalysis').checked=!!(s.workflow?.requirements?.dqo||s.workflow?.requirements?.surfactants);$('requiresSpecialAnalysis').disabled=false;$('saveBtn').textContent='Guardar cambios';matrixChanged();updateBranches()}
...
async function correctRegistryToStopped(id){
  try{
    const sample=allRows.find(x=>x.id===id);if(!sample)throw new Error('Muestra no encontrada.');
    if(!await qualityCorrectionGate.authorize({entityId:id,entityType:'SAMPLE',domain:'SAMPLES',label:`la muestra ${sample.code||sample.codeFull||''}`}))return;
    const reason=window.prompt(`MOTIVO DE LA CORRECCIÓN — obligatorio\n\nIndique por qué el estado CONTINUAR debe cambiarse a DETENIDA:`,'Corrección por error de registro');
    if(reason===null)return;
    await service.correctContinueToStopped(id,reason,{userId:qualityCorrectionGate.actor()});
    toast('Corrección aplicada: la muestra ahora está DETENIDA. El proceso y la trazabilidad se conservaron.');
    await refresh();
  }catch(e){toast(e.message||String(e),true)}
}
...
async function deleteClientCatalog(){const id=$('catalogClientId').value;if(!id)return;try{if(!await qualityDeleteGate.authorize({entityId:id,entityType:'CLIENT',domain:'CLIENTS',label:'este cliente'}))return;await service.deleteClientCatalog(id,{userId:qualityDeleteGate.actor()});toast('Cliente eliminado por Calidad. La trazabilidad fue conservada.');await refresh();clearClientForm()}catch(e){toast(e.message||String(e),true)}}
...
async function deleteMatrixCatalog(){const id=$('catalogMatrixId').value;if(!id)return;try{if(!await qualityDeleteGate.authorize({entityId:id,entityType:'MATRIX',domain:'CATALOGS',label:'esta matriz'}))return;await service.deleteMatrixCatalog(id,{userId:qualityDeleteGate.actor()});toast('Matriz eliminada por Calidad. La trazabilidad fue conservada.');await refresh();clearMatrixForm()}catch(e){toast(e.message||String(e),true)}}
...
async function safeDeleteSample(sampleId,stage){
  try{
    ...
    if(!await qualityDeleteGate.authorize({entityId:sampleId,entityType:'SAMPLE',domain:'SAMPLES',label:`el código ${code}`}))return;
    if(!confirm(`¿Eliminar ${code} del flujo?

Etapa actual: ${check.currentLabel}
Solo se aplicará borrado lógico coordinado. La auditoría y trazabilidad histórica se conservarán.`))return;
    const result=await lifecycleDeleteService.deleteFromCurrentStage(sampleId,stage,{userId:qualityDeleteGate.actor()});
    toast(`${result.code||code} eliminado por Calidad desde ${result.label}. Trazabilidad conservada.`);
    await refresh();
  }catch(e){toast(e.message||String(e),true)}
}
document.addEventListener('click',e=>{const a=e.target.closest('[data-analysis]'),w=e.target.closest('[data-waiting]'),h=e.target.closest('[data-history]'),ed=e.target.closest('[data-edit]'),qs=e.target.closest('[data-quality-stop]'),cc=e.target.closest('[data-client-catalog]'),mc=e.target.closest('[data-matrix-catalog]'),lo=e.target.closest('[data-lab-open]'),lsa=e.target.closest('[data-lab-save-analyst]'),lh=e.target.closest('[data-lab-history]'),sd=e.target.closest('[data-safe-delete-sample]');if(a)openAnalysis(a.dataset.analysis);if(w)openWaiting(w.dataset.waiting);if(h)openHistory(h.dataset.history);if(ed)openEdit(ed.dataset.edit);if(qs)correctRegistryToStopped(qs.dataset.qualityStop);if(cc)editClientCatalog(cc.dataset.clientCatalog);if(mc)editMatrixCatalog(mc.dataset.matrixCatalog);if(lo)openLabEntry(lo.dataset.labOpen);if(lsa)saveLabAnalyst(lsa.dataset.labSaveAnalyst);if(lh)openLabHistory(lh.dataset.labHistory);if(sd)safeDeleteSample(sd.dataset.safeDeleteSample,sd.dataset.safeDeleteStage)});document.addEventListener('change',e=>{const item=e.target.closest?.('[data-analysis-select]');if(item){if(item.checked)analysisSelected.add(item.dataset.analysisSelect);else analysisSelected.delete(item.dataset.analysisSelect);renderAnalysisBulkControls(queues.analysis.filter(x=>hay(x,$('searchAnalysis').value.trim().toLowerCase())));return}if(e.target.id==='analysisSelectAll'){const visible=queues.analysis.filter(x=>hay(x,$('searchAnalysis').value.trim().toLowerCase()));visible.forEach(x=>e.target.checked?analysisSelected.add(x.id):analysisSelected.delete(x.id));renderTables()}});document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')$('sampleForm').requestSubmit();if(e.altKey&&e.key.toLowerCase()==='n'){e.preventDefault();showView('register');clearForm()}if(e.altKey&&e.key.toLowerCase()==='r'){e.preventDefault();showView('register');repeatLast()}});
...
async function initializeAuthenticatedERP({alreadyInitialized=false}={}){
  ...
  if(alreadyInitialized){
    ...
    if(liveSyncManager){
      ...
      await refresh({syncDerived:false});
