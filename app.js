const APP_VERSION='V1.0.5.6.18-ALERTAS-TECNICAS-INTELIGENTES';
...
function technicalAlertActorIsAnalyst(plan){
  if(!currentSessionUser||currentSessionUser.role!=='ANALISTA')return false;
...
async function createTechnicalAlert(plan,kind,payload={}){
  ...
  const rec={id:uid('COM'),planId:plan.id,analystId:plan.analystId,analystName:plan.analystName,authorType:'ANALISTA',authorName:plan.analystName,text,createdAt:nowISO(),threadStatus:'OPEN',readBy:[],notificationType:'TECNICA',technicalKind:kind,priority,actionRequired:action,autoGenerated:true};
