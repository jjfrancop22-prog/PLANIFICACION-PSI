const APP_VERSION='V1.0.5.6.33.25.9-CENTRO-COMUNICACIONES-INTELIGENTE-CARTAS-CONSOLIDADAS';
...
async function assertDailyControlsBeforeDayClose(plan){if(!plan||plan.section!=='RECEPCION_MUESTRAS')return {ok:true};const dayPlans=(await visiblePlanningRows()).filter(x=>x.date===plan.date&&x.analystId===plan.analystId&&x.status!=='CANCELADO'),remaining=dayPlans.filter(x=>x.id!==plan.id&&x.status!=='REALIZADO');if(remaining.length)return {ok:true};const st=await dailyControlStatus(plan.date,plan.analystId);if(!st.pending.length)return {ok:true,status:st};return {ok:false,status:st,message:`No puede cerrar la jornada. Controles obligatorios pendientes (${st.done}/${st.total}): ${st.pending.map(x=>x.name).join(', ')}.`}}
...
async function recoverMissingBalanceEnvironmentDef(){
  ...
  const hist=records.filter(r=>r.controlType==='BALANZA_AMBIENTE_EI285').sort((a,b)=>String(b.measuredAt||'').localeCompare(String(a.measuredAt||'')))[0];
...
async function microFreezerRecords(chartId){return (await getAll('controlChartRecords')).filter(r=>r.chartId===chartId&&r.controlType==='CONGELADOR_MICROBIOLOGIA_EI350').sort((a,b)=>String(a.measuredAt).localeCompare(String(b.measuredAt)));}
...
async function microEnvRecords(chartId){return (await getAll('controlChartRecords')).filter(r=>r.chartId===chartId&&r.controlType==='MICROBIOLOGIA_AMBIENTE_EI347').sort((a,b)=>String(a.measuredAt).localeCompare(String(b.measuredAt)));}
...
async function balanceEnvRecords(chartId){return (await getAll('controlChartRecords')).filter(r=>r.chartId===chartId&&r.controlType==='BALANZA_AMBIENTE_EI285').sort((a,b)=>String(a.measuredAt).localeCompare(String(b.measuredAt)));}
...
async function metalsRecords(chartId){return (await getAll('controlChartRecords')).filter(r=>r.chartId===chartId&&r.controlType==='METALES_EI313').sort((a,b)=>String(a.measuredAt).localeCompare(String(b.measuredAt)));}
...
function fridgeRecords(chartId){return getAll('controlChartRecords').then(rows=>rows.filter(r=>r.chartId===chartId&&(r.controlType==='NEVERA_EI269'||r.controlType==='NEVERA_MICROBIOLOGIA_EI69')).sort((a,b)=>String(a.measuredAt).localeCompare(String(b.measuredAt))));}
