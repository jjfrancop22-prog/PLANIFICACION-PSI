const APP_VERSION='V1.0.5.6.43-CARTAS-RECEPCION-IA-PRO';
...
async function normalizeLegacyChartOutbox(){
  // Las cartas se mantienen locales en esta rama para no alterar el Firebase estable de 6.29.
  // Conservamos trazabilidad del Outbox antiguo, pero no dejamos que bloquee la sincronización operativa.
  const all=await getAll('outbox');
  for(const item of all){
    if(['controlChartEntries','controlChartConfig'].includes(item.entity) && ['PENDIENTE','ERROR'].includes(item.status)){
      item.status='OMITIDO';
      item.lastError='Carta de control local · no enviada al Firebase operativo';
