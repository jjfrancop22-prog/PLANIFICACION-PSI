...
async function renderCommunications(){
  ...
  if(!ts.length){$('#communicationsList').innerHTML=`<div class="comm-empty"><span>🔔</span><b>${filter==='UNREAD'?'No hay mensajes nuevos':'Sin conversaciones'}</b><small>${filter==='UNREAD'?'Cuando haya un comentario, una respuesta o un registro técnico nuevo, aparecerá aquí y la campana mostrará el aviso.':'No hay novedades para este filtro.'}</small></div>`;$$('[data-comm-filter]').forEach(b=>b.onclick=()=>{$('#commStatusFilter').value=b.dataset.commFilter;renderCommunications()});return}
