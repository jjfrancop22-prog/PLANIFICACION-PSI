...
async function savePlan(){
  ...
  if(conflict){
    ...
    const suggestion=slot?` Siguiente espacio disponible: ${minutesToTime(slot.start)}–${minutesToTime(slot.end)}.`:'';
    const box=$('#recommendationBox');
    if(box){box.className='recommendation-box error';box.innerHTML=`<b>Asignación bloqueada por seguridad</b><span>La hora elegida ${escapeHtml(start)} se cruza con ${escapeHtml(conflict.catalogName)} (${conflict.startTime}–${conflict.endTime}).${escapeHtml(suggestion)} Cambie la hora o use la sugerencia inteligente.</span>`;}
  ...
  if(addWorkingMinutes(startMin,dur)>WORK_END)return toast('No se guardó: la actividad excede la jornada laboral de 17:00. Cambie hora, duración, analista o fecha.');
  ...
  if(load+dur>capacity)return toast(`No se guardó: ${analyst.name} quedaría con ${minutesText(load+dur)} / ${analyst.dailyHours||8} h. La sobrecarga diaria está bloqueada.`);
...
async function renderMandatoryPlanningAlerts(){
  ...
  // REGLA 1: Recepción conserva cobertura diaria, pero la duración ya no es fija.
  // Se usa la duración configurada por el jefe en el catálogo seleccionado.
  ...
  if(!receptionPlans.length){
    ...
    const receptionMinutes=Math.max(5,Number(receptionCatalog?.baseMinutes||300));
    const eligible=active.filter(a=>(a.competencies||[]).includes('RECEPCION_MUESTRAS')).map(a=>({a,slot:findBestWorkSlot(plans,a.id,receptionMinutes),load:plannerRuleLoad(plans,a.id)})).filter(x=>x.slot).sort((a,b)=>a.load-b.load||a.slot.start-b.slot.start);
    if(!eligible.length){
      alerts.push({level:'danger',icon:'!',title:'PROGRAMAR RECEPCIÓN · SIN ESPACIO DISPONIBLE',text:`No existe Recepción de Muestras planificada y ningún analista competente conserva un bloque laboral de ${minutesText(receptionMinutes)}. Ajuste la duración, cambie de analista/fecha o libere espacio.`,meta:`Cobertura de Recepción · ${minutesText(receptionMinutes)}`});
    }else{
      const best=eligible[0],only=eligible.length===1;
      alerts.push({level:'warning',icon:'⌛',title:only?'PROGRAMAR RECEPCIÓN · ÚLTIMA OPCIÓN DISPONIBLE':'PROGRAMAR RECEPCIÓN DE MUESTRAS',text:`Aún no existe el bloque de Recepción configurado en ${minutesText(receptionMinutes)}. ${only?'Solo queda una opción':'Hay '+eligible.length+' opciones'} con espacio completo: ${best.a.name} ${minutesToTime(best.slot.start)}–${minutesToTime(best.slot.end)}.`,meta:`Cobertura de Recepción · ${minutesText(receptionMinutes)}`,action:{section:'RECEPCION_MUESTRAS',catalogId:receptionCatalog?.id||'',analystId:best.a.id,label:'Preparar Recepción'}});
    }
  }
...
async function latestConfirmedContainerRecord(reagent,container,excludePlanId=null){
  // INVENTARIO GLOBAL: nunca filtrar por usuario/rol. El saldo pertenece al lote, no al analista.
  const plans=await getAll('planning');let best=null;
...
async function renderFinishReagents(p){
  // Antes de mostrar cantidades, reconstruir el saldo global confirmado para evitar stock obsoleto por caché/PC/rol.
...
function updateCatalogForm(){const section=$('#catalogSection').value,meta=sectionMeta(section),timeSel=$('#catalogTimeMode'),forcedComposite=['RECEPCION_MUESTRAS','MICROBIOLOGIA','AASS'].includes(section);$('#familyLabel').childNodes[0].nodeValue=meta.family;if(forcedComposite){timeSel.value='COMPOSITE';timeSel.disabled=true}else{timeSel.disabled=false}const mode=timeSel.value;$('#baseMinutesLabel').classList.toggle('hidden',!['FIXED','COMPOSITE'].includes(mode));$('#catalogBaseHours').disabled=false;$('#catalogBaseMinutePart').disabled=false;$('#rulesEditor').classList.toggle('hidden',mode!=='BY_SAMPLES');$('#compositeEditor').classList.toggle('hidden',mode!=='COMPOSITE');const help=$('#compositeHelp');if(help)help.textContent=section==='RECEPCION_MUESTRAS'?'Defina libremente la duración total de Recepción de Muestras y distribuya ese tiempo entre sus subactividades. El Planificador reservará exactamente el bloque configurado.':section==='MICROBIOLOGIA'?'Defina la duración total de la actividad microbiológica y distribúyala entre sus subactividades. El futuro Planificador reservará el bloque completo con un solo clic.':section==='AASS'?'Defina la duración total del bloque de Absorción Atómica y distribúyala entre sus subactividades. El futuro Planificador reservará todo el bloque con un solo clic.':'Divida la duración total entre las subactividades que componen este bloque.';renderRuleRows();renderStepRows();updateCalibrationEditor();updateReagentEditor()}
...
function compositeTarget(){return getDurationPicker()}
...
function reagentInventorySignature(rows){return JSON.stringify((rows||[]).map(r=>({id:r.id||'',name:normalizeIdentityText(r.name||''),lot:normalizeIdentityText(r.lot||''),mode:r.mode||'COUNT',unit:normalizeIdentityText(r.unit||''),stock:r.mode==='COUNT'?Number(r.stockQuantity||0):null,status:r.inventoryStatus||'',containers:(r.containers||[]).map(e=>({id:e.id||'',label:normalizeIdentityText(e.label||''),lot:normalizeIdentityText(e.lot||''),tare:Number(e.tareWeight||0),initial:Number(e.initialWeight||0),status:e.status||''}))})).sort((a,b)=>(a.name+'|'+a.lot).localeCompare(b.name+'|'+b.lot)));}
function requireQualityInventoryPassword(){const password=prompt('Edición protegida de inventario. Ingrese contraseña de CALIDAD:');if(password===null)return false;if(password!=='CALIDAD'){toast('Contraseña de CALIDAD incorrecta');return false}return true}
async function saveCatalog(ev){ev.preventDefault();const id=$('#catalogId').value,section=$('#catalogSection').value,name=$('#catalogName').value.trim(),family=$('#catalogFamily').value.trim(),timeMode=$('#catalogTimeMode').value,baseMinutes=getDurationPicker();if(!section||!name)return toast('Complete sección y nombre');if(['FIXED','COMPOSITE'].includes(timeMode)&&!baseMinutes)return toast('Ingrese la duración estándar');if(timeMode==='BY_SAMPLES'){const v=validateRules(editingRules);if(v.level==='ERROR')return toast(v.text)}if(timeMode==='COMPOSITE'){const v=validateSteps();if(v.level==='ERROR')return toast(v.text)}if($('#catalogRequiresCalibration')?.checked){const v=validateCalibrationConfig();if(v.level==='ERROR')return toast(v.text)}if(sectionAllowsReagents(section)&&$('#catalogUsesReagents')?.checked){const v=validateReagents();if(v.level==='ERROR')return toast(v.text)}const all=await getAll('catalog');const duplicate=all.find(x=>x.id!==id&&x.section===section&&x.name.trim().toLowerCase()===name.toLowerCase()&&(x.family||'').trim().toLowerCase()===family.toLowerCase());if(duplicate)return toast('Ya existe el mismo elemento en esta sección y clasificación');const existing=id?all.find(x=>x.id===id):null;
// El inventario es un dato controlado. Una edición manual de reactivos/lotes/stock no puede
// sobrescribir silenciosamente el saldo confirmado por los analistas.
if(existing){
  const liveBefore=await liveReagentConfigForEditor(existing.reagentConfig||[]);
  const draft=reagentConfigFromForm();
  if(reagentInventorySignature(liveBefore)!==reagentInventorySignature(draft) && !requireQualityInventoryPassword())return;
}
const rec={id:id||uid('CAT'),code:existing?.code||nextCode(section,all),section,name,family,timeMode,baseMinutes:['FIXED','COMPOSITE'].includes(timeMode)?baseMinutes:null,description:$('#catalogDescription').value.trim(),calibrationConfig:calibrationConfigFromForm(),reagentConfig:reagentConfigFromForm(),status:$('#catalogStatus').value,createdAt:existing?.createdAt||nowISO(),updatedAt:nowISO()};await put('catalog',rec);const oldRules=(await getAll('timeRules')).filter(r=>r.catalogId===rec.id);for(const r of oldRules){await del('timeRules',r.id);await queue('DELETE','timeRules',{id:r.id})}if(timeMode==='BY_SAMPLES'){const seenRuleKeys=new Set();for(const r of editingRules){const key=timeRuleKey(r);if(seenRuleKeys.has(key))continue;seenRuleKeys.add(key);const ruleRec={id:uid('TR'),catalogId:rec.id,minSamples:Number(r.minSamples),maxSamples:Number(r.maxSamples),minutes:Number(r.minutes),createdAt:nowISO(),updatedAt:nowISO()};await put('timeRules',ruleRec);await queue('CREATE','timeRules',ruleRec)}}const oldSteps=(await getAll('compositeSteps')).filter(r=>r.catalogId===rec.id);for(const st of oldSteps){await del('compositeSteps',st.id);await queue('DELETE','compositeSteps',{id:st.id})}if(timeMode==='COMPOSITE'){for(let i=0;i<editingSteps.length;i++){const st=editingSteps[i];const stepRec={id:uid('STEP'),catalogId:rec.id,order:i+1,name:String(st.name).trim(),minutes:Number(st.minutes),createdAt:nowISO(),updatedAt:nowISO()};await put('compositeSteps',stepRec);await queue('CREATE','compositeSteps',stepRec)}}await queue(id?'UPDATE':'CREATE','catalog',rec);const propagated=id?await propagateCatalogTechnicalConfigToOpenPlans(rec):0;await audit(id?'EDITAR':'CREAR','CATALOGO_MAESTRO',rec.code,`${sectionMeta(section).label}: ${name}${family?` · ${family}`:''}${timeMode==='COMPOSITE'?` · bloque ${minutesText(baseMinutes)} con ${editingSteps.length} detalles`:''}`);currentSection=section;$('#catalogDialog').close();toast(id?(propagated?`Elemento actualizado · ${propagated} actividad(es) abierta(s) sincronizada(s)`:'Elemento actualizado'):'Elemento creado');await refreshAll();renderSectionTabs();
...
// 6.33.25.10.14 · Informe PDF controlado MC1602-08 (solo 3 cartas autorizadas)
function ccPdfAuthorized(def){return !!def&&(isBalanceEnvironmentChart(def)||isDBO5Chart(def)||isMicrobiologyEnvironmentChart(def));}
function ccPdfTitle(def){if(isBalanceEnvironmentChart(def))return 'CARTA DE CONTROL DE CONDICIONES DE BALANZA';if(isMicrobiologyEnvironmentChart(def))return 'CARTA DE CONTROL DE CONDICIONES DE MICROBIOLOGÍA';return 'CARTA DE CONTROL DE DBO5';}
function ccPdfEsc(v){return escapeHtml(String(v??'—'));}
function ccPdfNum(v,d=2){const n=Number(v);return Number.isFinite(n)?n.toFixed(d):'—';}
function ccPdfUpdateButton(def){const b=$('#btnCcPdf');if(!b)return;const ok=ccPdfAuthorized(def);b.classList.toggle('hidden',!ok);b.disabled=!ok;b.title=ok?'Generar informe mensual controlado MC1602-08 y guardar como PDF':'Disponible solo para las tres cartas autorizadas';}
async function exportControlChartMC160208(){
  const popup=window.open('','_blank');if(!popup){toast('Permita ventanas emergentes para generar el PDF MC1602-08');return;}
  popup.document.write('<title>Preparando MC1602-08…</title><p style="font-family:Arial;padding:30px">Preparando informe técnico…</p>');
  const id=$('#ccChartSelect')?.value,month=$('#ccMonth')?.value||ccMonthNow(),def=await getOne('controlChartDefs',id);
  if(!ccPdfAuthorized(def)){popup.close();return toast('Este formato PDF solo aplica a Condiciones de Balanza, DBO5 y Condiciones de Microbiología.');}
  const rows=(await dbo5Records(def.id)).filter(r=>String(r.measuredAt||'').startsWith(month));
  if(!rows.length){popup.close();return toast('No existen registros del período seleccionado para generar el informe.');}
  const tv=rows.map(r=>Number(r.tempCorrected)).filter(Number.isFinite),hv=rows.map(r=>Number(r.humCorrected)).filter(Number.isFinite),tw=dbo5Westgard(tv),hw=dbo5Westgard(hv);
  const cfg=isBalanceEnvironmentChart(def)?balanceEnvConfig(def):isMicrobiologyEnvironmentChart(def)?microEnvConfig(def):null;
  const tmin=cfg?.tempCriterion?.min??17,tmax=cfg?.tempCriterion?.max??23,hmax=cfg?.humCriterion?.max??80,hmin=Number.isFinite(cfg?.humCriterion?.min)?cfg.humCriterion.min:20;
  const compliant=rows.filter(r=>(r.environmentalResult||r.overallResult)==='CUMPLE').length,pct=100*compliant/rows.length;
  const [yy,mm]=month.split('-'),monthLabel=new Intl.DateTimeFormat('es-EC',{month:'long',year:'numeric'}).format(new Date(Number(yy),Number(mm)-1,1));
  const tempChart=$('#ccTempChart')?.innerHTML||'',humChart=$('#ccHumChart')?.innerHTML||'',ai=$('#ccAi')?.innerText||'',rules=$('#ccRules')?.innerText||'';
  const eq=isBalanceEnvironmentChart(def)?'EI-285':(rows[0]?.equipment||rows[0]?.thermohygrometer||def.equipmentKey||'—');
  const logger=rows[0]?.dataLogger||'PF-09';
  const tr=rows.map((r,i)=>`<tr><td>${i+1}</td><td>${ccPdfEsc(String(r.measuredAt||'').slice(0,16).replace('T',' '))}</td><td>${ccPdfEsc(r.analystName)}</td><td>${ccPdfNum(r.tempRaw)}</td><td>${ccPdfNum(r.tempFactor)}</td><td>${ccPdfNum(r.tempCorrected)}</td><td>${ccPdfNum(r.humRaw,1)}</td><td>${ccPdfNum(r.humFactor,1)}</td><td>${ccPdfNum(r.humCorrected,1)}</td><td><b>${ccPdfEsc(r.environmentalResult||r.overallResult)}</b></td><td>${ccPdfEsc((r.statisticalRules||[]).join(', ')||'—')}</td></tr>`).join('');
  const title=ccPdfTitle(def),generated=new Intl.DateTimeFormat('es-EC',{dateStyle:'medium',timeStyle:'short'}).format(new Date());
  popup.document.open();popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>MC1602-08 · ${title} · ${month}</title><style>
  @page{size:A4 landscape;margin:9mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#172033;margin:0;font-size:9px}.head{display:grid;grid-template-columns:20% 60% 20%;border:1.4px solid #172033}.head>div{padding:8px;border-right:1px solid #172033;display:flex;align-items:center;justify-content:center;text-align:center}.head>div:last-child{border:0}.brand{font-weight:800;font-size:17px;letter-spacing:.5px}.title{font-weight:800;font-size:14px}.code{font-weight:800;font-size:12px}.meta{display:grid;grid-template-columns:repeat(6,1fr);border:1px solid #8a96a8;border-top:0}.meta div{padding:6px;border-right:1px solid #c8ced8}.meta div:last-child{border:0}.meta small{display:block;color:#65738a;font-size:7px;text-transform:uppercase}.meta b{display:block;margin-top:2px}.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin:8px 0}.k{border:1px solid #c8ced8;border-radius:5px;padding:6px}.k small{display:block;color:#65738a}.k b{font-size:13px}.charts{display:grid;grid-template-columns:1fr 1fr;gap:8px}.chart{border:1px solid #c8ced8;border-radius:5px;padding:6px;min-height:150px}.chart h3{font-size:10px;margin:0 0 3px}.chart svg{width:100%!important;height:135px!important}.section{font-size:10px;margin:8px 0 4px;border-bottom:1px solid #172033;padding-bottom:3px}table{width:100%;border-collapse:collapse;font-size:7.2px}th,td{border:1px solid #aeb7c5;padding:3px;text-align:center}th{background:#eef2f7}.analysis{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}.box{border:1px solid #c8ced8;border-radius:5px;padding:7px;white-space:pre-line;line-height:1.35}.sign{display:grid;grid-template-columns:1fr 1fr 1fr;gap:18px;margin-top:20px}.sign div{text-align:center;border-top:1px solid #172033;padding-top:4px}.foot{margin-top:8px;font-size:7px;color:#65738a;display:flex;justify-content:space-between}.no-print{position:fixed;right:12px;top:12px;padding:10px 16px;background:#172033;color:white;border:0;border-radius:6px;font-weight:700}@media print{.no-print{display:none}.pagebreak{break-before:page}.chart,.box,tr{break-inside:avoid}}
  </style></head><body><button class="no-print" onclick="window.print()">Guardar / Imprimir PDF</button>
  <header class="head"><div class="brand">LAB-PSI</div><div class="title">${title}<br><span style="font-size:9px;font-weight:normal">Registro mensual de seguimiento, tendencia y vigilancia estadística</span></div><div class="code">MC1602-08<br><span style="font-size:8px">Registro controlado</span></div></header>
  <div class="meta"><div><small>Período</small><b>${ccPdfEsc(monthLabel)}</b></div><div><small>Equipo</small><b>${ccPdfEsc(eq)}</b></div><div><small>Data logger / patrón</small><b>${ccPdfEsc(logger)}</b></div><div><small>Criterio temperatura</small><b>${ccPdfEsc(tmin)}–${ccPdfEsc(tmax)} °C</b></div><div><small>Criterio humedad</small><b>${Number.isFinite(hmin)?ccPdfEsc(hmin)+'–':''}≤ ${ccPdfEsc(hmax)} %HR</b></div><div><small>Generado</small><b>${ccPdfEsc(generated)}</b></div></div>
  <div class="kpis"><div class="k"><small>Controles</small><b>${rows.length}</b></div><div class="k"><small>Cumplimiento</small><b>${pct.toFixed(1)}%</b></div><div class="k"><small>T media ± s</small><b>${ccPdfNum(mean(tv))} ± ${ccPdfNum(sd(tv))} °C</b></div><div class="k"><small>HR media ± s</small><b>${ccPdfNum(mean(hv),1)} ± ${ccPdfNum(sd(hv),1)} %</b></div><div class="k"><small>Estado</small><b>${tw.state==='CONTROL'&&hw.state==='CONTROL'?'EN CONTROL':'VIGILANCIA'}</b></div></div>
  <div class="charts"><div class="chart"><h3>Temperatura corregida · Shewhart/Westgard</h3>${tempChart}</div><div class="chart"><h3>Humedad corregida · Shewhart/Westgard</h3>${humChart}</div></div>
  <h2 class="section">Trazabilidad de registros del período</h2><table><thead><tr><th>N°</th><th>Fecha/hora</th><th>Responsable</th><th>T lectura °C</th><th>Factor T</th><th>T corregida °C</th><th>HR lectura %</th><th>Factor HR</th><th>HR corregida %</th><th>Resultado</th><th>Reglas / señales</th></tr></thead><tbody>${tr}</tbody></table>
  <div class="analysis"><div class="box"><b>ANÁLISIS DEL PERÍODO</b><br>${ccPdfEsc(ai)}</div><div class="box"><b>REGLAS ESTADÍSTICAS / DECISIÓN DE CALIDAD</b><br>${ccPdfEsc(rules)}<br><br><b>Resumen:</b> ${compliant}/${rows.length} controles cumplen los criterios técnicos definidos. La vigilancia estadística se interpreta de forma independiente del cumplimiento técnico.</div></div>
  <div class="sign"><div>Elaborado / registrado por<br><b>Analista(s) del período</b></div><div>Revisado por<br><b>Jefe / Coordinador de Calidad</b></div><div>Aprobación / fecha<br><b>________________________</b></div></div>
  <div class="foot"><span>MC1602-08 · Documento generado desde ERP Planificación NextGen · ${ccPdfEsc(APP_VERSION)}</span><span>Página 1 · ${ccPdfEsc(month)}</span></div>
  <script>setTimeout(()=>window.print(),500)<\/script></body></html>`);popup.document.close();
  await audit('EXPORTAR_PDF','CARTA_CONTROL',def.id,`MC1602-08 · ${title} · ${month} · ${rows.length} registros`);toast('Informe MC1602-08 preparado. Seleccione “Guardar como PDF”.');
}
